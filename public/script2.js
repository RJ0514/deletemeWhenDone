...
async function convert(){
    ...
    else if (input == 0)
    {
       ...
       // integer = parseInt(integer);
        ...
        console.log(answer_bin,' ',answer_hex);
        //Soln: 1: uSE SLeep
        //location.reload();
        //await sleep(5000);

        //Soln2: Use localStoreage
        //localStorage.setItem("binNum",answer_bin);
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            const { hexNum, binNum } = JSON.parse(xhr.responseText);
            document.getElementById("hexNum").value = hexNum;
            document.getElementById("binNum").value = binNum;
          }
        };
        xhr.send(JSON.stringify({ binary: normalized_results[0]}))

        binNum.innerHTML = answer_bin
        hexNum.innerHTML = answer_hex;
        //location.reload();
        //console.log(localStorage.getItem("binNum"));
    }
    else if (base == "10"){
...
function writeToFile() {
    const output = `Binary: ${binNum.innerHTML}\nHexadecimal: ${hexNum.innerHTML}`;
